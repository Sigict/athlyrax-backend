...
async function startSparseLegacyServer() {
...
test('sparse legacy Schedule deletes through the real tenant route and a stale whole-db PUT cannot resurrect it', async () => {
  const server = await startSparseLegacyServer();
  try {
    ...
    const persisted = readJson(server.tenantDbPath);
    assert.equal(scheduleIds(persisted).has('sparse-old-schedule'), false, 'tenant db.json resurrected sparse Schedule');
    assert.equal((persisted?.trainingSessions || []).some((row) => row?.id === 'sparse-rendered-session'), false, 'tenant db.json resurrected linked Training Session');
