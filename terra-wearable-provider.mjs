...
function unbase64url(value) { return Buffer.from(String(value ?? ''), 'base64url').toString('utf8'); }
...
export function buildTerraPlannedWorkout(workout = {}) {
  ...
  const steps = asArray(workout.sets).map((set, index) => {
    ...
    const descriptionParts = [
      text(set.title), positiveNumber(set.rounds) ? `${Number(set.rounds)} rounds` : '', positiveNumber(set.reps) ? `${Number(set.reps)} reps` : '',
      text(set.stroke), text(set.sendoff) ? `send-off ${text(set.sendoff)}` : '', text(set.rest) ? `rest ${text(set.rest)}` : '', text(set.target),
    ].filter(Boolean);
    return {
      type: 'STEP', order: index + 1, name: text(set.title || `Set ${index + 1}`), description: descriptionParts.join(' · '),
      intensity: 'ACTIVE', exercise_type: 'SWIMMING', durations: distanceMeters ? [{ duration_type: 'DISTANCE', distance_meters: distanceMeters }] : [], targets: [],
    };
  });
  return { data: [{ metadata: { summary_id: sessionId, name: text(workout.title || 'AthlyraX training session'), planned_date: text(workout.date) }, steps }] };
}

export function buildTerraPlannedWorkoutRequest({ workout, terraUserId, apiKey, devId, apiBaseUrl = TERRA_API_BASE_URL } = {}) {
  const userId = text(terraUserId); const key = text(apiKey); const developerId = text(devId);
  ...
  return {
    ...
    init: { method: 'POST', headers: { accept: 'application/json', 'content-type': 'application/json', 'x-api-key': key, 'dev-id': developerId }, body: JSON.stringify(buildTerraPlannedWorkout(workout)) },
  };
}

export function parseTerraPlannedWorkoutResponse(payload = {}) {
  const ids = asArray(payload.log_ids).map(text).filter(Boolean);
  return { ok: ids.length > 0, externalWorkoutId: ids[0] || '', externalWorkoutIds: ids, error: ids.length > 0 ? '' : text(payload.message || payload.error || 'Terra did not return a workout log id.') };
}
...
export function buildTerraWidgetRequest({ referenceId, apiKey, devId, successUrl = '', failureUrl = '', language = 'en', apiBaseUrl = TERRA_API_BASE_URL } = {}) {
  const reference = text(referenceId); const key = text(apiKey); const developerId = text(devId);
  ...
  return {
    ...
    init: { method: 'POST', headers: { accept: 'application/json', 'content-type': 'application/json', 'x-api-key': key, 'dev-id': developerId }, body: JSON.stringify({ language: text(language || 'en') || 'en', reference_id: reference, ...(text(successUrl) ? { auth_success_redirect_url: text(successUrl) } : {}), ...(text(failureUrl) ? { auth_failure_redirect_url: text(failureUrl) } : {}) }) },
  };
}

export function createAthlyraxTerraReference({ username, tenantId, secret } = {}) {
  const user = text(username).toLowerCase(); const tenant = text(tenantId); const key = text(secret);
...
export function parseAthlyraxTerraReference(referenceId, secret) {
  const reference = text(referenceId); const key = text(secret); const [payload, signature, extra] = reference.split('.');
  ...
  try {
    ...
    const username = text(parsed?.u).toLowerCase(); const tenantId = text(parsed?.t);
...
export function upsertTerraConnection(db = {}, { username, tenantId, terraUserId, provider, referenceId, active = true, updatedAt = '' } = {}) {
  const user = text(username).toLowerCase(); const tenant = text(tenantId); const terraId = text(terraUserId); const resource = text(provider).toLowerCase();
  if (!user || !tenant || !terraId || !resource) return { ok: false, error: 'Complete Terra connection identity is required.' };
  const rows = asArray(db.athleteWearableConnections); const key = `terra:${tenant}:${user}:${resource}`; const previous = rows.find((row) => text(row?.key) === key) || {};
  const connection = { ...previous, key, gateway: TERRA_PROVIDER_ID, provider: resource, username: user, tenantId: tenant, terraUserId: terraId, referenceId: text(referenceId), active: active !== false, updatedAt: text(updatedAt) || new Date().toISOString() };
  return { ok: true, connection, db: { ...db, athleteWearableConnections: [...rows.filter((row) => text(row?.key) !== key), connection] } };
}

export function findTerraConnection(db = {}, { username, tenantId, provider = '' } = {}) {
  const user = text(username).toLowerCase(); const tenant = text(tenantId); const resource = text(provider).toLowerCase();
  return asArray(db.athleteWearableConnections).find((row) => text(row?.gateway).toLowerCase() === TERRA_PROVIDER_ID && text(row?.username).toLowerCase() === user && text(row?.tenantId) === tenant && row?.active !== false && (!resource || text(row?.provider).toLowerCase() === resource)) || null;
}
...
export function verifyTerraSignature({ rawBody, signatureHeader, signingSecret, nowSeconds = Math.floor(Date.now() / 1000), toleranceSeconds = 300 } = {}) {
  const signature = text(signatureHeader); const secret = text(signingSecret);
...
export function parseTerraAuthEvent(payload = {}) {
  ...
  const terraUserId = text(user.user_id); const referenceId = text(payload.reference_id || user.reference_id); const provider = text(user.provider).toLowerCase();
