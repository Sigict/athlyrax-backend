...
function setIdOf(row = {}) {
  return text(row.id || row.setId);
}
...
export function buildCanonicalWearableWorkout(db = {}, sessionId = '') {
  ...
  if (text(session.approvalStatus).toLowerCase() === 'pending') {
    return { ok: false, status: 409, error: 'Pending athlete proposal cannot be sent to a wearable.' };
  }
  if (text(session.approvalStatus).toLowerCase() === 'rejected') {
    return { ok: false, status: 409, error: 'Rejected athlete proposal cannot be sent to a wearable.' };
  }
  ...
  return {
    ...
    workout: {
      ...
      sets: sets.map((row, index) => ({
        ...
        title: text(row.title || row.name || `Set ${index + 1}`),
        rounds: row.rounds ?? null,
        reps: row.reps ?? null,
        distance: row.distance ?? null,
        distanceUnit: text(row.distanceUnit || 'm'),
        stroke: text(row.stroke),
...
function deliveryKey(providerId, sessionId) {
  return `${text(providerId).toLowerCase()}:${text(sessionId)}`;
}
...
export function beginWearableDelivery(db = {}, { providerId, sessionId, attemptedAt = '' } = {}) {
  ...
  return {
    ...
    db: {
      ...db,
      athleteWearableDeliveries: [...rows.filter((row) => text(row?.key) !== key), delivery],
    },
  };
}

export function finishWearableDelivery(db = {}, {
  providerId,
  sessionId,
  ok,
  externalWorkoutId = '',
  ...
  const sessions = asArray(db.trainingSessions).map((row) => {
    if (sessionIdOf(row) !== id) return row;
    return {
      ...row,
      wearableSync: {
        ...
        externalWorkoutId: delivery.externalWorkoutId,
        lastError: delivery.lastError,
        lastSyncedAt: delivery.lastSyncedAt,
      },
    };
  });
...
export function mergeWearableExecutionIntoDb(db = {}, sessionId = '', execution = {}) {
  const id = text(sessionId);
  if (!id || text(execution.sessionId) !== id) {
    return { ok: false, status: 400, error: 'Wearable execution must target the same canonical session.' };
  }
  const session = asArray(db.trainingSessions).find((row) => sessionIdOf(row) === id);
  if (!session) return { ok: false, status: 404, error: 'Canonical training session was not found.' };
  ...
  for (const setId of returnedById.keys()) {
    if (!validSetIds.has(setId)) return { ok: false, status: 409, error: 'Wearable returned a set outside the canonical session.' };
  }
  ...
  const sessions = asArray(db.trainingSessions).map((row) => {
    ...
    return {
      ...
      wearableSync: {
        ...
        externalWorkoutId: text(execution.externalWorkoutId || row?.wearableSync?.externalWorkoutId),
        lastSyncedAt: now,
      },
      updatedAt: now,
    };
